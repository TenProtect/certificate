...
public class OrderController {
    ...
    /**
     * Get nickname and avatar of the user who created the order.
     * This endpoint does not require authentication.
     */
    @GetMapping("/share-info/{id}")
    public Map<String, String> getOrderShareInfo(@PathVariable("id") @Positive Integer id) {
        OrderDO order = orderService.getById(id);
        if (order == null) {
            throw new NotFoundException(110000);
        }
        UserDO user = userService.getById(order.getUserId());
        if (user == null) {
