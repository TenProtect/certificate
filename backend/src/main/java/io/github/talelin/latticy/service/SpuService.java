...
public class SpuService extends ServiceImpl<SpuMapper, SpuDO> {
    ...
    public void create(SpuDTO dto) {
        ...
        Integer cid = dto.getCategoryId();
        if (cid == null) {
            cid = 0;
        }
        spuDO.setCategoryId(cid);
        if (cid == 0) {
            spuDO.setRootCategoryId(0);
        } else {
            CategoryDO categoryDO = categoryService.getCategoryById(cid);
    ...
    public void update(SpuDTO dto, Integer id) {
        ...
        if (cid == 0) {
            ...
            CategoryDO category = categoryService.getCategoryById(cid);
    ...
    public IPage<SpuDO> pageWithMerchant(Integer count, Integer page) {
        // MyBatis-Plus expects 1-based page number; convert from 0-based API page
        Page<SpuDO> pager = new Page<>(page + 1, count);
        return this.getBaseMapper().selectPageWithMerchant(pager);
    }
