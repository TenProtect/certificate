...
function getLocaleLanguage$1() {
  let localeLanguage = "";
  {
    localeLanguage = normalizeLocale(my.getSystemInfoSync().language) || LOCALE_EN;
  }
...
const offError = {
  args(fromArgs) {
    ...
    if (!app.$vm) {
      ...
      const index2 = my.$onErrorHandlers.findIndex((fn) => fn === fromArgs);
      if (index2 !== -1) {
        my.$onErrorHandlers.splice(index2, 1);
      }
...
const request = {
  ...
  args(fromArgs) {
    ...
    return {
      ...
      data(data) {
        if (isDingDing && method.toUpperCase() === "POST" && headers["content-type"].indexOf("application/json") === 0) {
...
function initRuntimeSocketService() {
  const hosts = "172.19.84.237,192.168.31.179,192.168.23.1,192.168.189.1,127.0.0.1";
  const port = "8090";
  const id2 = "mp-alipay__Dox4S";
...
function findPropsData(properties, isPage) {
  return (isPage ? findPagePropsData(properties) : findComponentPropsData(resolvePropValue(properties.uP))) || {};
}
